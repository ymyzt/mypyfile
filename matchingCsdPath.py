	...
	if csd :#文件不为空
		...
		while Path != -1:
			...
			if Path != -1:
				...
				if Normal == -1:
					G_ERROR.write(file+"中没找到 Type=\n")
				t1 = csd.find("\"",Normal+1)
				t2 = csd.find("\"",t1+1)
				NO = csd[t1+1:t2]
				Plist = csd.find("Plist=",Normal)
				if Plist == -1:
					G_ERROR.write(file+"没找到 Plist= \n")
				l1 = csd.find("\"",Plist+1)
				l2 = csd.find("\"",l1+1)
				LIST = csd[l1+1:l2]
				if ".plist" in LIST:
		 		...
		 		if not isfind:
					if ".jpg" in png :
						jpg = FindSanPng(png)
						if len(jpg) == 0:
							G_ERROR.write("没有找到 "+png+" 相关路径上的jpg!!\n")
						elif len(jpg) == 1:
							csd = csd[:p1+1]+jpg[0]+csd[p2:]
						elif len(jpg)>1 :
							G_ERROR.write(file+" 里 "+png+ " 路径上存在多张图片:\n")
							for a in range(len(jpg)):
								G_ERROR.write(str(a)+" "+jpg[a]+"\n")
						Normal = csd.rfind(" Type=",0,Path)
						t1 = csd.find("\"",Normal+1)
						t2 = csd.find("\"",t1+1)
						...
						if len(addpng)>1:
							...
							for a in range(len(addpng)):
									if hasbig and a == 0:
										G_ERROR.write(str(a)+" "+big["plist"]+"中 "+addpng[a]+"\n")
									else:
										G_ERROR.write(str(a)+" "+addpng[a]+"\n")
						elif len(addpng) == 1 :
							p = addpng[0]
							if hasbig:
								l = big["plist"]
							...
							G_ERROR.write(file+"里没找到 "+png+" 相关路径上的图片！\n")
						csd = csd[:p1+1]+p+csd[p2:]
						Normal = csd.rfind(" Type=",0,Path)
						if Normal == -1:
							G_ERROR.write(file+"里没找到 Type=\n")
						t1 = csd.find("\"",Normal+1)
						t2 = csd.find("\"",t1+1)
						csd = csd[:t1+1]+no+csd[t2:]
						Plist = csd.find("Plist=",Normal)
						if Plist == -1:
							G_ERROR.write(file+"里没找到 Plist=\n")
						l1 = csd.find("\"",Plist+1)
						l2 = csd.find("\"",l1+1)
		...
		#print(file+"文件为空!\n")
		G_ERROR.write(file+"文件为空!\n")
	return csd

#####

####
def FindBigPng(string): #找大图
	...
	for i in idx:
		for num in range(0,4):#plist最多4张
			...
			if os.path.exists("../cocosstudio/res/"+plist):
			 	...
			 	if fi.find(str_ch)!=-1:
			 		if dir.has_key("png"):
			 			G_ERROR.write("在"+dir["plist"]+" 和res/"+plist+" 中都存在"+dir["png"])
			 		else:	
